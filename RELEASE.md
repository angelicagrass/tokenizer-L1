...
| -------------------  | ---------------------------------------------|
|Tokenizer.findBestMatchOfTokenValue | Som boken nämner **bör if blocket enbart innhålla en rad**, tex ett anrop till en funktion som gör det som ligger inuti if blocket nu. Sen skulle den metoden i sin tur göra flera olika saker vilket inte heller är optimalt då boken förspråker att funktionerna ska vara väldigt små och **enbart göra en sak.** I övrigt tycker jag att den ändå är ganska enkel att förstå trots att den är lite större. Namngivningen känns bra och förkortningen av tempArray känns ändå som den bordde gå att förstå, men det är ju mitt antagande och kanske inte alls känns lika självklart för någon annans synvinkel att det just är en temporär array. Namnet på själva funktionen kan dock förbättras, vad är best? Vad är den bästa matchningen? Lite väl öppet för tolkningar.
|  returnErrorObject()   |    Denna metoden returnerar ett object med information om att tokenmatchningen inte lyckades med dee angiva regex-reglerna. Här hade man kan ske vilja ha ettv tydligare namn till exempel. ReturnErrorObject säger inget om när detta inträffar, vid vilket fel anropas just denna funktionen? Här skulle man kunna bryta ut så att man tömmer variablen this.tokenizeString i en egen funktion då jag kan tänka mig när denna klass växer så kan man säkert behöva ha nytta av en egen funktion som tömmer just den variabeln. Då följer man också bokens exempel på att en funktion enbart **bör göra en sak**.                                         |
TheTokenizer.getPreviousToken() | Denna metoden sätter aktivt token till föregående token. Det gör att namngivningen här är förvirrande och något jag kommer att ändra. Den heter just getPreviousToken och den hämtar ingen token utan den sätter enbart om det aktiva tokenet till föregående. Vill man hämta det token som är aktivt så är det isåfall metoden getActiveToken man använder sig utav. Här har det blivit bra med att **funktionen är väldigt liten.**
Grammar.createNewGrammar() | Denna methoden skapar ett object med hjälp av parametrarna som skickas in i klassen Grammar.Objectet består av typen och regex som behövs för att skapa ett nyttset av regler till sin tokenizer. I denna skickar man alltså in vilken typ av regex mamn vill skapa. Här kommer jag vilja skapa någon form av loopning så man ska kunna skicka in flera regex för att skapa sitt nya set av regler men på grund av tidsbrist har inte detta blivit implementerat än. Namngivningen är förvirrande då den enbart heter Grammar, som Daniel Toll föreslog kanske Grammarbuilder skulle vara något att föredra då det för användaren kan vara svårt utan en förklaring att veta vad den egenligen gör, namnet missvisar och man kanske mer tror att den innehåller färdiga grammars eller dylikt. |
